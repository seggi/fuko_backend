...
def upgrade():
    ...
    sa.Column('periode_range', sa.Text(), nullable=True),
    ...
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    sa.ForeignKeyConstraint(['currency_id'], ['currency.id'], ),
    sa.ForeignKeyConstraint(['memeber_id'], ['group_members.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.add_column('dept_note_book', sa.Column('friend_id', sa.Integer(), nullable=True))
    op.drop_constraint('dept_note_book_borrower_id_fkey', 'dept_note_book', type_='foreignkey')
    op.create_foreign_key(None, 'dept_note_book', 'notebook_member', ['friend_id'], ['id'])
    op.drop_column('dept_note_book', 'borrower_id')
    op.add_column('loan_note_book', sa.Column('friend_id', sa.Integer(), nullable=True))
    op.drop_constraint('loan_note_book_partner_id_fkey', 'loan_note_book', type_='foreignkey')
    op.create_foreign_key(None, 'loan_note_book', 'notebook_member', ['friend_id'], ['id'])
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('loan_note_book', sa.Column('partner_id', sa.INTEGER(), autoincrement=False, nullable=True))
    op.drop_constraint(None, 'loan_note_book', type_='foreignkey')
    op.create_foreign_key('loan_note_book_partner_id_fkey', 'loan_note_book', 'users', ['partner_id'], ['id'])
    op.drop_column('loan_note_book', 'friend_id')
    op.add_column('dept_note_book', sa.Column('borrower_id', sa.INTEGER(), autoincrement=False, nullable=True))
    op.drop_constraint(None, 'dept_note_book', type_='foreignkey')
    op.create_foreign_key('dept_note_book_borrower_id_fkey', 'dept_note_book', 'users', ['borrower_id'], ['id'])
