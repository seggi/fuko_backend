...
def get_sent_request():
    ...
    manage_request =  ManageRequest(schema=[user_schema, user_create_group_schema], data=[*get_request])
...
def request_status():
    ...
    try:
        ...
        if data["request_status"] == REQUEST_CANCELED:
            group = db.session.query(GroupMembers).filter(GroupMembers.id == data['id']).one()
